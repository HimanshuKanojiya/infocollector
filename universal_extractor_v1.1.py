...
import codecs
from googlesearch import search as searchme
...
def initiator_quick(url):
    urls = []
    robos = extracts(url, url)
    urls_a = url.partition('//')[-1]
    find = searchme("site:"+urls_a, num=100, stop=None, pause=40.0)
...
if __name__ == "__main__":
    ...
    os.system("CLS")
    print(banner)
    print("\n Created By Himanshu Kanojiya, From India\n * Current Version: {}".format("1.1"))

    if str('v1.1') == str(version_ver):
        ...
        while True:
            ...
            sitemap_internal = []
            if len(url)!= 0:
                search = input("Press: {} for quick method, it will take less time(less results).\nPress: {} for long method, it will take time(More results).\n Warning: Quick method may cause to block your 'IP'!, Use 'VPN' for it.\n\n Type(1/2) > ".format(1,2))
